...
as $function$
    ...
    where
        ...
        and (
            p_min_beds is null
            or csl.beds >= p_min_beds
        )

        and (
            p_max_beds is null
            or csl.beds <= p_max_beds
        )

        and (
            p_min_baths is null
            or csl.baths >= p_min_baths
        )

        and (
            p_max_baths is null
            or csl.baths <= p_max_baths
        )

        and (
            p_min_price is null
            or csl.current_price >= p_min_price
        )

        and (
            p_max_price is null
            or csl.current_price <= p_max_price
        )
...
create function public.filtered_property_snapshot(
    p_group_level text,
