...
export default async function DevelopmentPage({
  ...
  if (error || drilldownError || profileError) {
    return (
...
function StickyBreadcrumb({
  ...
  return (
    <div className="sticky top-0 z-40 border-b border-slate-800 bg-slate-700 px-4 py-3 text-white">
      <div className="mx-auto max-w-6xl overflow-hidden text-center text-sm font-bold">
...
function NearbySection({
  ...
  const nearbySummary = cleanText(profile?.nearby_summary);
  const nearbyNotes = cleanText(profile?.nearby_notes);

  const hasNearbyStats = [
    profile?.nearby_restaurants_count,
    profile?.nearby_bars_count,
    profile?.nearby_galleries_count,
    profile?.nearby_cafes_count,
    profile?.nearby_beach_minutes,
    profile?.nearby_grocery_minutes,
  ].some((value) => value !== null && value !== undefined);

  const shouldShowNearby = nearbySummary || nearbyNotes || hasNearbyStats;
  ...
  return (
    <div style={{ paddingTop: "48px" }}>
      <section className="rounded-3xl border border-slate-200 bg-white p-8 shadow">
        ...
        <h2 className="mt-2 text-2xl font-bold">Around {developmentName}</h2>

        {shouldShowNearby ? (
          <>
            {nearbySummary && (
              <p className="mt-4 leading-7 text-slate-700">{nearbySummary}</p>
            )}
            ...
            {hasNearbyStats && (
              <div className="mt-6 grid grid-cols-2 gap-4 md:grid-cols-3">
                <NearbyStat
                  label="Restaurants"
                  value={profile?.nearby_restaurants_count}
                />
                <NearbyStat label="Bars" value={profile?.nearby_bars_count} />
                <NearbyStat
                  label="Galleries"
                  value={profile?.nearby_galleries_count}
                />
                <NearbyStat label="Cafés" value={profile?.nearby_cafes_count} />
                <NearbyStat
                  label="Beach Walk"
                  value={
                    profile?.nearby_beach_minutes
                      ? `${profile.nearby_beach_minutes} min`
                      : null
                  }
                />
                <NearbyStat
                  ...
                  value={
                    profile?.nearby_grocery_minutes
                      ? `${profile.nearby_grocery_minutes} min`
                      : null
                  }
                />
              </div>
            ...
            {nearbyNotes && (
              <p className="mt-5 leading-7 text-slate-700">{nearbyNotes}</p>
            )}
          </>
        ) : (
          <p className="mt-4 leading-7 text-slate-700">
            Nearby place data has not yet been added for {developmentName}.
            This section is designed to show restaurants, cafés, bars, galleries,
            beach access, groceries, and other lifestyle conveniences near{" "}
            {communityName}, {areaName} as the SearchPV location database is
            expanded.
          </p>
        )}
      </section>
...
function ProfileTextSection({ title, text }: { title: string; text: string }) {
  return (
    <section className="mt-8 border-t border-slate-200 pt-6">
      ...
      <p className="mt-3 whitespace-pre-line leading-7 text-slate-700">{text}</p>
    </section>
  );
}
...
function MetricCard({
  ...
  return (
    <div className="rounded-3xl border border-slate-100 bg-white px-8 py-7 shadow-sm">
