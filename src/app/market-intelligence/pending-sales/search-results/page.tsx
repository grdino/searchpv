...
export const metadata: Metadata = {
  title: "Pending Sales Search Results | SearchPV",
  robots: {
    index: false,
    follow: true,
  },
...
export default async function PendingSalesSearchResultsPage({
  searchParams,
}: {
  ...
  let query = supabase.from("pending_listing").select(`
    mls,
    zone_name,
    area_name,
    community_name,
  ...
  const rows = (data ?? []) as PendingListingRow[];
        ...
        <div className="mx-auto max-w-7xl px-4 py-8 md:px-8">
          ...
          <div className="mt-8">
            ...
            <h1 className="mt-2 text-3xl font-bold md:text-4xl">
              Pending Sales Search Results
            </h1>

            <p className="mt-3 max-w-3xl text-slate-300">
              Current pending MLS listings matching the selected market,
        ...
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3 rounded-xl bg-white p-4 shadow">
          <div>
            ...
            <p className="text-2xl font-bold text-slate-950">
              {rows.length.toLocaleString("en-US")} pending listing
              {rows.length === 1 ? "" : "s"}
            </p>
          </div>
...
function buildReturnHref({
  ...
  const queryString = params.toString();
  const basePath = "/market-intelligence/pending-sales";
