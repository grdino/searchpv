...
export default async function ClosedSalesByAgencyPage({
  searchParams,
}: {
  ...
  const rpcParams = {
    ...
    p_zone_nm: selectedZone === "all" ? null : selectedZone,
    p_area_nm: selectedArea === "all" ? null : selectedArea,
    p_community_nm: selectedCommunity === "all" ? null : selectedCommunity,
    p_development_nm: selectedDevelopment === "all" ? null : selectedDevelopment,
    p_property_type_cd: toRpcPropertyType(selectedPropertyType),
    p_market_type_nm: toRpcMarketType(selectedMarket),
  };
  ...
  if (error) {
    return (
      <main className="min-h-screen bg-slate-50 p-8">
        <h1 className="text-3xl font-bold">Closed Sales by Agency</h1>
        <p className="mt-4 text-red-600">Error loading the agency report.</p>
  ...
  const zones = uniqueSorted(optionRows.map((r) => r.zone_name));
  const areas = uniqueSorted(optionRows.filter((r) => selectedZone === "all" || r.zone_name === selectedZone).map((r) => r.area_name));
  const communities = selectedArea === "all" ? [] : uniqueSorted(optionRows.filter((r) => selectedZone === "all" || r.zone_name === selectedZone).filter((r) => r.area_name === selectedArea).map((r) => r.community_name));
  const developments = selectedCommunity === "all" ? [] : uniqueSorted(optionRows.filter((r) => selectedZone === "all" || r.zone_name === selectedZone).filter((r) => selectedArea === "all" || r.area_name === selectedArea).filter((r) => r.community_name === selectedCommunity).map((r) => r.development_name));
    ...
    <main className="min-h-screen bg-slate-50 text-slate-900">
      <section className="bg-slate-950 px-4 py-10 text-white md:px-8 md:py-14">
        <div className="mx-auto max-w-7xl">
          ...
          <h1 className="mt-2 text-3xl font-bold">Closed Sales by Agency</h1>
          <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-300">Compare actual transaction volume with listing-side, selling-side, both-sides, and total side production.</p>

          <AgencyClosedSalesFilters
      ...
      <div className="sticky top-0 z-40 border-b border-slate-800 bg-slate-700 px-4 py-3">
        <div className="mx-auto max-w-7xl text-center text-sm font-bold text-white">
          <div className="truncate">
            ...
            {" > "}
            <Link href="/office" className="underline hover:text-sky-200">Office</Link>
            {" > "}
            <span>Closed Sales by Agency</span>
          </div>
          <div className="mt-1 text-xs font-semibold text-slate-200">
            {formatSelectedFilters({ selectedMarket, selectedPropertyType, selectedZone, selectedArea, selectedCommunity, selectedDevelopment, selectedStartDate, selectedEndDate })}
          </div>
        </div>
      </div>
      ...
      <section className="mx-auto max-w-7xl px-4 py-6 md:px-8 md:py-10">
        <div className="mb-6 rounded-xl bg-white p-5 shadow">
          <p className="text-sm font-semibold text-slate-500">Showing Closed Sales From</p>
          <p className="mt-1 text-lg font-bold">{formatDateLong(selectedStartDate) || "All Time"} through {formatDateLong(selectedEndDate) || "Today"}</p>
        </div>
          ...
          <div id="ranking-table" className="mt-3 max-h-[72vh] scroll-mt-16 overflow-auto rounded-xl bg-white shadow">
            <table className="min-w-[1800px] border-separate border-spacing-0 text-sm">
              <thead className="bg-slate-100 text-slate-700">
                <tr>
                  <SortableTh label="Rank" sortKey="rank" rankColumn />
                  ...
                  <SortableTh label="List Sides" sortKey="listing_sides" />
                  <SortableTh label="List Volume" sortKey="listing_volume_usd" />
                  <SortableTh label="Sell Sides" sortKey="selling_sides" />
                  <SortableTh label="Sell Volume" sortKey="selling_volume_usd" />
                  <SortableTh label="Both Sides" sortKey="both_sides" />
                  <SortableTh label="Total Sides" sortKey="total_sides" />
                  <SortableTh label="Side Volume" sortKey="total_side_volume_usd" />
                  <SortableTh label="Capture %" sortKey="side_capture_pc" />
                  <SortableTh label="Both-Sides %" sortKey="both_sides_pc" />
                  <SortableTh label="Median Sold" sortKey="median_sold_price_usd" />
                  <SortableTh label="Median DOM" sortKey="median_dom" />
                  <SortableTh label="Median Sold/List" sortKey="median_sold_to_list_pc" />
                </tr>
              </thead>
              <tbody>
                {rows.map((row, index) => (
                  <tr key={row.agency_nm}>
                    ...
                    <Td>{formatNumber(row.listing_sides)}</Td>
                    <Td>{formatMoney(row.listing_volume_usd)}</Td>
                    <Td>{formatNumber(row.selling_sides)}</Td>
                    <Td>{formatMoney(row.selling_volume_usd)}</Td>
                    <Td>{formatNumber(row.both_sides)}</Td>
                    <Td>{formatNumber(row.total_sides)}</Td>
                    <Td>{formatMoney(row.total_side_volume_usd)}</Td>
                    <Td>{formatPercent(row.side_capture_pc)}</Td>
                    <Td>{formatPercent(row.both_sides_pc)}</Td>
                    <Td>{formatMoney(row.median_sold_price_usd)}</Td>
                    <Td>{formatNumber(row.median_dom, 1)}</Td>
                    <Td>{formatPercent(row.median_sold_to_list_pc)}</Td>
                  </tr>
                ))}
                {rows.length === 0 && <tr><td colSpan={16} className="px-4 py-10 text-center text-slate-500">No agency results match the selected filters.</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
        ...
        <div className="mt-6 rounded-xl border border-slate-200 bg-white p-5 text-sm text-slate-600">
          <p className="font-bold text-slate-900">How the volume measures differ</p>
          <p className="mt-2 leading-6">Transaction volume counts each property once for an agency. Total side volume counts the sold price once for every side represented. A $1,000,000 transaction where the same agency represented both parties contributes $1,000,000 of transaction volume and $2,000,000 of side volume.</p>
        </div>
      </section>
    </main>
  );
  ...
  function SortableTh({
    ...
    const defaultDir: SortDir = sortKey === "rank" ? "asc" : "desc";
    ...
    return <th className={className}><Link href={`${href}#ranking-table`} className="hover:underline">{label}{arrow}</Link></th>;
  }
}

function KpiGrid({ summary }: { summary: AgencyReportSummary | null }) {
  const cards = [
    ["Closed Transactions", formatNumber(summary?.closed_transactions), "Actual properties sold"],
    ["Transaction Volume", formatMoney(summary?.transaction_volume_usd), "Each sale counted once"],
    ["Listing Sides", formatNumber(summary?.listing_sides), formatMoney(summary?.listing_volume_usd)],
    ["Selling Sides", formatNumber(summary?.selling_sides), formatMoney(summary?.selling_volume_usd)],
    ["Both Sides", formatNumber(summary?.both_sides), `${formatPercent(summary?.both_sides_pc)} of transactions`],
    ["Total Sides", formatNumber(summary?.total_sides), `${formatPercent(summary?.side_capture_pc)} capture rate`],
    ["Total Side Volume", formatMoney(summary?.total_side_volume_usd), "Listing + selling volume"],
    ["Median Sold Price", formatMoney(summary?.median_sold_price_usd), `Average ${formatMoney(summary?.average_sold_price_usd)}`],
...
function formatSelectedFilters({ selectedMarket, selectedPropertyType, selectedZone, selectedArea, selectedCommunity, selectedDevelopment, selectedStartDate, selectedEndDate }: { selectedMarket: string; selectedPropertyType: string; selectedZone: string; selectedArea: string; selectedCommunity: string; selectedDevelopment: string; selectedStartDate: string; selectedEndDate: string }) {
  const parts: string[] = [];
  parts.push(selectedMarket === "all" ? "All Markets" : selectedMarket === "pre_construction" ? "Pre-Construction" : "Resale");
  parts.push(selectedPropertyType === "all" ? "All Property Types" : selectedPropertyType === "condos" ? "Condos" : "Houses");
  parts.push(selectedZone === "all" ? "All Zones" : selectedZone);
  if (selectedArea !== "all") parts.push(selectedArea);
  if (selectedCommunity !== "all") parts.push(selectedCommunity);
  if (selectedDevelopment !== "all") parts.push(selectedDevelopment);
  parts.push(`${formatDateShort(selectedStartDate) || "All Time"} – ${formatDateShort(selectedEndDate) || "Today"}`);
  return parts.join(" • ");
}
