...
import { buildMarketListingUrl, MARKET_DASHBOARDS, type MarketDashboardConfig } from "@/lib/market-dashboard/listing-links";
...
type MarketData = {
  ...
  readiness?: {
    ...
    instruments: Record<string, "ready" | "limited">;
    samples: { active: number; sold12m: number; sold5y: number; yearsWithSales: number; inventoryObservations: number; sellerBehavior: number };
  };
  sellerBehavior: {
...
export default function InteractiveMarketDashboard({ market }: { market: MarketDashboardConfig }) {
  ...
  useEffect(() => {
    ...
    const nextUrl = `/market/${marketSlug}?${params.toString()}`;
    window.history.replaceState(window.history.state, "", nextUrl);
  }, [marketSlug, propertyType, bedrooms, segment]);
  ...
  useEffect(() => {
    ...
    fetch(`/api/market-dashboard/${marketSlug}?${params.toString()}`, { signal: controller.signal })
      .then(async (response) => {
  ...
  const dashboardStateUrl = buildDashboardStateUrl(marketSlug, propertyType, bedrooms, segment);
  const matchingPropertiesUrl = buildMatchingPropertiesUrl(market, propertyType, bedrooms, segment, dashboardStateUrl);
  const availableListingsUrl = buildMarketListingUrl(marketSlug, {
    status: "active",
    propertyType,
    bedrooms,
    segment,
  ...
  const pendingListingsUrl = buildMarketListingUrl(marketSlug, {
    status: "pending",
    propertyType,
    bedrooms,
    segment,
  ...
  return (
    <main className={styles.page}>
      ...
      <section className={styles.hero}>
        ...
        <h1>{market.displayName} <span>Real Estate Market</span></h1>
        <p className={styles.snapshot}>{market.breadcrumb.join(" / ")}</p>
        <section className={styles.marketBrief}>
          <div>
            <span>MARKET BRIEF</span>
            <p>{market.marketBrief}</p>
          </div>
        </section>
      ...
      <div className={styles.stickyMarket} aria-label="Current market selection">
        <div className={styles.stickyIdentity}>
          <label className={styles.marketSelectorLabel}>
            <span>Market</span>
            <select
              className={styles.marketSelector}
              value={marketSlug}
              aria-label="Market dashboard"
              onChange={(event) => {
                const nextMarket = event.target.value;
                if (nextMarket === "__all__") {
                  router.push("/markets");
                  return;
                }
                const params = new URLSearchParams({ property: propertyType, bedrooms, segment });
                router.push(`/market/${nextMarket}?${params.toString()}`);
              }}
            >
              {Object.values(MARKET_DASHBOARDS)
                .filter((candidate) => candidate.published)
                .map((candidate) => (
                  <option key={candidate.slug} value={candidate.slug}>{candidate.displayName}</option>
                ))}
              <option value="__all__">All Market Dashboards →</option>
            </select>
          </label>
          <b>{data?.asOf ? `Data through ${longDate(data.asOf)}` : "Loading market data…"}</b>
        </div>
        {data?.readiness?.status === "limited" ? (
          <div className={styles.selectionReadiness} title="One or more dashboard instruments have a small sample for the current filters.">
            Limited data for this selection
          </div>
        ) : data?.readiness?.status === "empty" ? (
          <div className={styles.selectionReadiness} title="No active, pending, or trailing-12-month closed sales match the current filters.">
      ...
      {data?.readiness?.status === "empty" ? (
        <section className={styles.noActivityNotice} aria-live="polite">
          <div>
            <b>No recent market activity for this selection</b>
            <p>There are currently no active or pending {market.displayName} {bedrooms === "All" ? "" : `${bedrooms} `}{segment === "Both" ? "" : `${segment.toLowerCase()} `}{propertyType.toLowerCase()} listings in the MLS, and no matching closed sales were recorded in the trailing 12 months.</p>
          </div>
          <div className={styles.broadenActions} aria-label="Broaden this market selection">
            {bedrooms !== "All" ? <button type="button" onClick={() => setBedrooms("All")}>All bedrooms</button> : null}
            {(bedrooms !== "All" || segment !== "Both") ? <button type="button" onClick={() => { setBedrooms("All"); setSegment("Both"); }}>All {propertyType.toLowerCase()}s</button> : null}
            {propertyType === "House" ? <button type="button" onClick={() => { setPropertyType("Condo"); setBedrooms("All"); setSegment("Both"); }}>Condos</button> : null}
          </div>
        </section>
      ) : null}
      ...
      <section className={styles.dashboard}>
        ...
        <AnimatedArticle id="price-range" className={`${styles.panel} ${styles.wide}`}>
          ...
          {data && data.current.active === 0 ? (
            <Empty text="No active listings match this selection." />
          ) : (
          <div data-ignition-target className={styles.bands}>
            {(data?.priceBands || []).map((row, index) => {
              ...
              const href = buildMarketListingUrl(marketSlug, {
                status: "active",
                propertyType,
                bedrooms,
                segment,
          ...
          {!loading && data?.priceBands.length === 0 && data.current.active > 0 ? <Empty /> : null}
        </AnimatedArticle>
        ...
        <AnimatedArticle id="market-composition" className={`${styles.panel} ${styles.wide}`}>
          ...
          <div data-ignition-target className={styles.composition}>
            {(data?.composition || []).map((row, index) => {
              const rowBedrooms = compositionBedroomFromLabel(row.label);
              const href = row.count > 0 && rowBedrooms
                ? buildMarketListingUrl(marketSlug, {
                    status: "active",
                    propertyType,
                    bedrooms: rowBedrooms,
                    segment,
                  })
                : null;
              const content = (
                <>
                  <span>{row.label}</span>
                  <div><i style={{ width: `${row.count / maxComposition * 100}%`, animationDelay: `${index * .08}s` }} /></div>
                  <b>{row.count} · {compositionTotal ? Math.round(row.count / compositionTotal * 100) : 0}%</b>
                </>
              );
              return href ? (
                <a
                  key={row.label}
                  href={href}
                  className={styles.compositionLink}
                  aria-label={`View ${row.count} active ${row.label} listings in ${market.displayName}`}
                >
                  {content}
                </a>
              ) : (
      ...
      <section className={styles.next}>
        <div><span>FROM MARKET → PROPERTY</span><h2>See the properties behind the numbers.</h2><p>Your current market view is already a property search: <b>{market.displayName} · {propertyType} · {bedrooms === "All" ? "All bedrooms" : bedrooms} · {segment}</b>.</p></div>
        <div className={styles.actions}><Link href={matchingPropertiesUrl}>Search matching properties →</Link><Link href={`/atlas?q=${encodeURIComponent(market.atlasQuery)}`}>Explore in Atlas →</Link></div>
      </section>
...
function compositionBedroomFromLabel(label: string): Bedrooms | null {
  if (label === "Studio") return "Studio";
  if (label === "1 BR") return "1 BR";
  if (label === "2 BR") return "2 BR";
  if (label === "3+ BR") return "3+ BR";
  return null;
}

function buildDashboardStateUrl(marketSlug: string, propertyType: PropertyType, bedrooms: Bedrooms, segment: Segment) {
  const params = new URLSearchParams({ property: propertyType, bedrooms, segment });
  return `/market/${marketSlug}?${params.toString()}`;
}
function buildMatchingPropertiesUrl(market: MarketDashboardConfig, propertyType: PropertyType, bedrooms: Bedrooms, segment: Segment, returnUrl: string) {
