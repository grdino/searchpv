...
function aggregateInventoryHistory(rows: any[]) {
  const byDate = new Map<string, any>();
  for (const row of rows) {
    if (!row?.snapshot_date) continue;
    const current = byDate.get(row.snapshot_date) || { snapshot_date: row.snapshot_date, active_listing_count: 0 };
    current.active_listing_count += Number(row.active_listing_count || 0);
    byDate.set(row.snapshot_date, current);
  }
  return [...byDate.values()].sort((a, b) => String(a.snapshot_date).localeCompare(String(b.snapshot_date)));
}

export async function GET(request: NextRequest, { params }: { params: Promise<{ marketSlug: string }> }) {
  ...
  const communities = geographies.map((g) => g.community);
  ...
  const applyInventoryFilters = (query: any) => {
    let q = query;
    if (sameZoneArea) {
      q = q.eq("zone_name", geographies[0].zone).eq("area_name", geographies[0].area).in("community_name", communities);
    } else {
      throw new Error("Multi-area dashboard geography is not supported yet.");
    }
    q = q.eq("prprty_type", propertyHistory);
  ...
  const applyClosedFilters = (query: any) => {
    ...
    q = q.eq("property_type_segment", propertyPublic);
    if (bedroom) q = q.eq("bedroom_segment", bedroom);
    if (marketSegment) q = q.eq("market_segment", marketSegment);
    return q;
  };
  ...
  const historyRequests = geographies.flatMap((geo) => {
    const args = {
    ...
    return [
      supabase.rpc("active_listing_history_summary", args),
      supabase.rpc("pending_sales_history_summary", args),
    ];
  });

  const [activeResult, pendingResult, closedResult, ...historyResults] = await Promise.all([
    activePromise,
    pendingPromise,
    closedPromise,
    ...historyRequests,
  ...
  const firstError = [activeResult, pendingResult, closedResult, ...historyResults]
    .map((r: any) => r.error)
    .find(Boolean);

  if (firstError) {
    console.error(`${market.displayName} market dashboard query failed`, firstError);
  ...
  const activeHistory = aggregateInventoryHistory(historyResults.filter((_, i) => i % 2 === 0).flatMap((r: any) => r.data || []));
  const pendingHistory = aggregateInventoryHistory(historyResults.filter((_, i) => i % 2 === 1).flatMap((r: any) => r.data || []));
  ...
  const yearsWithSales = new Set(closed.map((r: any) => String(r.sold_date).slice(0, 4))).size;
  const usableSellerRows = sellerRows.filter((r: any) =>
    Number.isFinite(Number(r.sold_to_final_list_pct)) ||
    Number.isFinite(Number(r.sold_to_original_list_pct))
  ).length;
  const readiness = {
    snapshot: active.length >= 5 ? "ready" : "limited",
    priceDistribution: active.length >= 10 ? "ready" : "limited",
    marketTrends12m: sold12.length >= 12 ? "ready" : "limited",
    marketTrendsLong: closed.length >= 30 && yearsWithSales >= 4 ? "ready" : "limited",
    inventoryTrend: active.length >= 10 && activeHistory.length >= 20 ? "ready" : "limited",
    sellerBehavior: usableSellerRows >= 12 ? "ready" : "limited",
    marketComposition: active.length >= 10 ? "ready" : "limited",
  } as const;
  const selectionLimited = Object.values(readiness).some((status) => status === "limited");
  const noRecentActivity = active.length === 0 && pending.length === 0 && sold12.length === 0;
  ...
  return NextResponse.json({
    geography: { ...GEO, publicName: market.displayName },
    ...
    readiness: {
      status: noRecentActivity ? "empty" : selectionLimited ? "limited" : "ready",
      instruments: readiness,
      samples: {
        active: active.length,
        sold12m: sold12.length,
        sold5y: closed.length,
        yearsWithSales,
        inventoryObservations: activeHistory.length,
        sellerBehavior: usableSellerRows,
      },
    },
    sellerBehavior: {
